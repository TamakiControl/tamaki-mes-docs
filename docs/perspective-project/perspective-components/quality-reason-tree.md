---
sidebar_position: 11
sidebar_label: Quality Reason Tree
title: Quality Reason Tree
---

## Overview

The Quality Reason Tree component is a custom Perspective component that provides a hierarchical tree view of quality reasons. It allows users to easily navigate and select quality reasons from a structured list, making it possible to categorize and analyze quality events. Unlike the other trees, this component can be passed a locationId to filter the reasons shown based on the location.

## Props

The Quality Reason Tree component has the following props:
...
The Quality Reason Tree component emits the following events:
...
| `onImport` | Emitted when the user imports quality reasons. | `None` |
| `onExport` | Emitted when the user exports quality reasons. | `{ currentItemId: string}` |
