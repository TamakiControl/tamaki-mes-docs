---
sidebar_position: 10
sidebar_label: Performance Reason Tree
title: Performance Reason Tree
---

## Overview

The Performance Reason Tree component is a custom Perspective component that provides a hierarchical tree view of performance reasons. It allows users to easily navigate and select performance reasons from a structured list, making it possible to categorize and analyze performance events. Unlike the other trees, this component can be passed a locationId to filter the reasons shown based on the location.

## Props

The Performance Reason Tree component has the following props:
...
The Performance Reason Tree component emits the following events:
...
| `onImport` | Emitted when the user imports performance reasons. | `None` |
| `onExport` | Emitted when the user exports performance reasons. | `{ currentItemId: string}` |
